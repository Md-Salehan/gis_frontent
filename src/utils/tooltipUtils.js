...
const generatePropertiesSection = (properties = {}) => {
  ...
  // Collect all images across all image properties into a single flat list
  const allImages = imageEntries
    .map(([key, value]) => {
      const urls = extractImageUrls(value).filter((u) => {
        try {
      ...
      return urls.map((u) => ({ url: u, key: formatKey(key) }));
    })
    .flat();

  // If no images and no non-image properties -> placeholder
  if (nonImageEntries.length === 0 && allImages.length === 0) {
    return `
  ...
  if (allImages.length > 0) {
    // If multiple images, render a single horizontal scrollable row
    if (allImages.length > 1) {
      const items = allImages
        .map(
          ...
          <a href="${
            it.url
          }" target="_blank" rel="noopener noreferrer" aria-label="${it.key} ${
            i + 1
        ...
        <div style="padding:12px 16px;border-bottom:1px solid #e5e7eb;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
            ...
            <div style="font-size:12px;color:#64748b;">${
              allImages.length
            } image${allImages.length > 1 ? "s" : ""}</div>
          </div>
      ...
      // Single image: show it with a property-specific heading if available
      const it = allImages[0];
      imagesHtml = `
        <div style="padding:12px 16px;border-bottom:1px solid #e5e7eb;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
            <div style="display:flex;align-items:center;gap:8px;">
              ...
              <h4 style="margin:0;font-size:13px;font-weight:600;color:#1e293b;">${it.key}</h4>
            </div>
          </div>
          <div style="display:flex;align-items:flex-start;">
            <a href="${it.url}" target="_blank" rel="noopener noreferrer" style="display:inline-block;border-radius:8px;overflow:hidden;box-shadow:0 6px 18px rgba(15,23,42,0.08);">
              <img loading="lazy" src="${it.url}" alt="${it.key}" style="display:block;height:160px;max-width:100%;width:auto;object-fit:cover;border:0;">
            </a>
          </div>
        </div>
      `;
