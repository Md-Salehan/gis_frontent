 ...
 * @param {Object} latlng - Leaflet LatLng object (centroid)
 * @param {Object} labelStyle - Label style configuration
 * @returns {L.Marker} Leaflet marker with custom label
 */
export const createLabelMarker = (feature, latlng, labelStyle = {}) => {
  const {
    label_text = "",
  ...
  // Build inline HTML for label with styling
  const html = `
    <div style="
  ...
  const icon = L.divIcon({
    ...
    html,
    iconSize: undefined, // Auto-size based on content
    iconAnchor: undefined, // Center the label
  });

  return L.marker(latlng, {
    icon,
    interactive: false,
    zIndexOffset: 1000, // Ensure labels appear above other features
  });
};
  ...
  try {
    switch (type) {
      ...
      case "Polygon":
        ...
        return calculatePolygonCentroid(coordinates[0]); // Use outer ring
...
/**
 * Get label configuration from feature properties
...
export const getLabelConfig = (properties = {}, metaData = {}) => {
  ...
  return {
    label_text: properties[labelColumn] || "xxxx",
    ...
    label_zoom_level: Number(styleConfig.label_zoom_level) || 14,
  };
};