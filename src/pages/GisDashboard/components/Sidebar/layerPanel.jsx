...
<Space><div className="polygon-icon"></div>{option.label}
      {disabled && <Spin size="small" style={{ marginLeft: 8 }} />}</Space>
    </Checkbox>
  </Col>
));
...
const LayerPanel = memo(({ layers = [], handleLayerToggle }) => {
  ...
  const onChange = useCallback(async (checkedValues) => {
    const previous = new Set(checkedState.checkedLayers);
    const current = new Set(checkedValues);

    // Handle newly checked layers
    const toFetch = checkedValues.filter(id => 
      !previous.has(id) && !geoJsonLayers[id]
    );
  ...
  }, [checkedState.checkedLayers, fetchLayerData, handleLayerToggle, geoJsonLayers]);
