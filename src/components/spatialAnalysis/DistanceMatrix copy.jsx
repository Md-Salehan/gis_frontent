...
function DistanceMatrix() {
  ...
  const buildKDIndex = useCallback((targetFeatures) => {
    ...
    targetFeatures.forEach((feature, index) => {
      ...
      if (coords) {
        ...
        // Store index in feature properties for reference
    ...
    return { index, points, featureMap };
  }, []);
  ...
  const calculateDistance = useCallback((coords1, coords2, unit) => {
    ...
    const distance = turf.distance(from, to, { units: unit });
    return distance;
  }, []);
  ...
  const processWithKDTree = useCallback(
    async (
      ...
      const { index, points, featureMap } = buildKDIndex(targetFeatures);
      ...
      for (let i = 0; i < totalSources; i++) {
        ...
        const nearest = geokdbush.around(index, lng, lat, k);
        ...
        for (const neighbor of nearest) {
          ...
          const targetFeature = neighbor.feature;
          ...
          const targetId =
            targetFeature.properties?.[targetIdKey] || neighbor.index;
    ...
    <Space direction="vertical" style={{ width: "100%" }} size={4}>
      ...
      <Card size="small" bodyStyle={{ padding: "8px 12px" }}>
        <Form layout="vertical" style={{ marginBottom: 0 }}>
          <Form.Item
            label={<Text style={{ fontSize: 12 }}>Source Layer (Points)</Text>}
            required
      ...
      <Card size="small" bodyStyle={{ padding: "8px 12px" }}>
        <Form layout="vertical" style={{ marginBottom: 0 }}>
          <Form.Item
            label={<Text style={{ fontSize: 12 }}>Target Layer (Points)</Text>}
            required
            style={{ marginBottom: 6 }}
          >
            <Select
          ...
          <Row gutter={[4, 2]}>
            ...
            <Col span={6}>
              <Statistic
                ...
                suffix={<Text style={{ fontSize: 9 }}>{resultStats.unit}</Text>}
                precision={4}
              />
            </Col>
            <Col span={6}>
