...
// Add this component inside PrintPreviewMap
const TileLoaderOptimizer = ({ scaleValue }) => {
  const map = useMap();
  
  useEffect(() => {
    if (!scaleValue) return;
    
    // Force tile reload at optimal zoom for export
    const optimizeTiles = () => {
      map.eachLayer((layer) => {
        if (layer._url && layer.redraw) {
          // Redraw tiles for crisp rendering
          layer.redraw();
        }
      });
    ...
    // Wait a bit then optimize
    const timer = setTimeout(optimizeTiles, 500);
    return () => clearTimeout(timer);
  }, [map, scaleValue]);
  ...
  ({ geoJsonLayers, bufferLayers, viewport, showLegend, orientation, format, scaleValue }, ref) => {
    ...
    return (
      <MapContainer {...mapSettings} ref={ref}>
        <TileLoaderOptimizer scaleValue={scaleValue} />
        <BaseMapTileLayer />
