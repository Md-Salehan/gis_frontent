...
// Expression validator with date support
const validateExpression = (expression, columns) => {
  const errors = [];
  ...
  // Validate date values if column is date type
  const datePattern = /"([^"]+)"\s*(=|!=|>|<|>=|<=)\s*'([^']+)'/g;
  while ((match = datePattern.exec(expression)) !== null) {
    ...
    if (column?.type === "date") {
      ...
      if (!/^\d{2}\/\d{2}\/\d{4}$/.test(value)) {
        errors.push(
          `Date value "${value}" for column "${columnName}" should be in DD/MM/YYYY format`,
        );
      } else {
        ...
        if (
          ...
          errors.push(
            `Invalid date "${value}" for column "${columnName}". Please use a valid date in DD/MM/YYYY format`,
          );
        }
      }
    }
...
const QueryBuilderAdvance = ({ activeTab, layerData, onApplyFilters }) => {
  ...
  const insertFieldValue = useCallback(() => {
    ...
    if (columnType === "string") {
      ...
      if (selectedValue.match(/^\d{2}\/\d{2}\/\d{4}$/)) {
        formattedValue = `'${selectedValue}'`;
      } else {
        message.error("Please select a valid date in DD/MM/YYYY format");
        return;
      }
  ...
  const handleApply = useCallback(() => {
    const validationResult = validateExpression(expression, columns);
  ...
  }, [expression, onApplyFilters, activeTab, columns]);
  ...
  const filteredDistinctValues = useMemo(() => {
    if (!searchValue) return distinctValues.slice(0, 100);
              ...
              options={columns.map((col) => ({
                label: (
                  <Space>
                    <span>{col.name}</span>
                    <Tag
                      color={getLabelTypeColor(col.type)}
                      style={{ fontSize: 10 }}
                    >
                      {col.type}
                    </Tag>
...
"name" LIKE '%School%' AND "established_date" = '15/08/1947'
"date_column" > '01/01/2020' AND "status" != 'closed'`}
          rows={6}
          style={{
            fontFamily: "monospace",
            fontSize: "13px",
