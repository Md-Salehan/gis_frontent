...
import { extractLeafletSVG } from "../../utils";
...
const PrintControl = () => {
  ...
  // In handleExportPDF function, update the layout calculation:
  const handleExportPDF = async (values) => {
    try {
      ...
      // 1️⃣ Extract SVG from Leaflet with proper dimensions
      const svgElement = extractLeafletSVG(mapContainer);
      ...
      const MARGIN_X = 10;
      const TOP_MARGIN = 10;
      const TITLE_HEIGHT = values.title ? 8 : 0;
      const FOOTER_HEIGHT = values.footerText ? 8 : 0;
      const SCALE_HEIGHT = values.mapScale ? 5 : 0;
      ...
      if (values.title) {
        ...
        pdf.text(values.title, pageWidth / 2, cursorY, { align: "center" });
      ...
      if (values.mapScale) {
        pdf.setFontSize(10);
        pdf.text(
          `Scale: 1:${values.mapScale.replace(/,/g, "")}`,
          pageWidth - MARGIN_X,
          cursorY,
          { align: "right" }
        );
        cursorY += SCALE_HEIGHT;
      }

      // ================================
      // MAP DRAWING AREA
      // ================================
      const mapTopY = cursorY + 2; // Small padding
      const mapBottomY = pageHeight - FOOTER_HEIGHT - 2;

      const mapAvailableWidth = pageWidth - MARGIN_X * 2;
      const mapAvailableHeight = mapBottomY - mapTopY;

      // Calculate scale to fill available space while maintaining aspect ratio
      const svgAspectRatio = svgWidth / svgHeight;
      const availableAspectRatio = mapAvailableWidth / mapAvailableHeight;

      let finalWidth, finalHeight, offsetX, offsetY;

      if (svgAspectRatio > availableAspectRatio) {
        // SVG is wider than available space
        finalWidth = mapAvailableWidth;
        finalHeight = finalWidth / svgAspectRatio;
        offsetX = MARGIN_X;
        offsetY = mapTopY + (mapAvailableHeight - finalHeight) / 2;
      } else {
        // SVG is taller than available space
        finalHeight = mapAvailableHeight;
        finalWidth = finalHeight * svgAspectRatio;
        offsetX = MARGIN_X + (mapAvailableWidth - finalWidth) / 2;
        offsetY = mapTopY;
      }
      ...
      const scaleFactor = Math.min(
        finalWidth / (svgWidth * pxToMm),
        finalHeight / (svgHeight * pxToMm)
      );

      // ================================
      // DRAW SVG (MAP) - Center and scale properly
      // ================================
      try {
        ...
        console.error("SVG to PDF conversion error:", svgError);
        // Fallback: Draw a placeholder
        pdf.setFillColor(240, 240, 240);
        pdf.rect(offsetX, offsetY, finalWidth, finalHeight, "F");
      ...
      if (values.footerText) {
        ...
        pdf.text(values.footerText, pageWidth / 2, pageHeight - 4, {
          align: "center",
        });
      }
      ...
      const fileName = values.title
        ? `${values.title.replace(/\s+/g, "_")}_${new Date()
            .toISOString()
            .slice(0, 10)}.pdf`
