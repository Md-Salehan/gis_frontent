...
function BufferTool() {
  ...
  const singleSelected = useSelector(
    (s) => s.map.selectedFeature.feature || []
  );
  ...
  const selectedFeatures = useMemo(() => {
    if (multiSelected && multiSelected.length > 0) {
      ...
      return multiSelected.map((m) => ({ ...m }));
    }
    if (singleSelected && singleSelected.length > 0) {
      // singleSelected is an array of features
      return singleSelected.map((f, idx) => ({
