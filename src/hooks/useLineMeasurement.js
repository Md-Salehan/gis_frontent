    ...
    const handleDoubleClick = (e) => {
      ...
      try {
        ...
        if (finalLatLngs.length > 0) {
          ...
          // if there isn't a marker at the last point, add one
          const lastMarker = vertexMarkersRef.current.length
            ? vertexMarkersRef.current[vertexMarkersRef.current.length - 1]
            : null;
          // Always add marker for last point to guarantee start and end visible
